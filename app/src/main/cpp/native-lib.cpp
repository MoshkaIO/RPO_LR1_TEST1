...
Java_com_example_rpo_1lr1_1test1_MainActivity_initRng(JNIEnv *env, jclass clazz) {
    // TODO: implement initRng()
        mbedtls_entropy_init( &entropy );
    mbedtls_ctr_drbg_init( &ctr_drbg );
...
Java_com_example_rpo_1lr1_1test1_MainActivity_randomBytes(JNIEnv *env, jclass clazz, jint no) {
    uint8_t * buf = new uint8_t [no];
    mbedtls_ctr_drbg_random(&ctr_drbg, buf, no);
    jbyteArray rnd = env->NewByteArray(no);
    env->SetByteArrayRegion(rnd, 0, no, (jbyte *)buf);
    delete[] buf;
    return rnd;
    // TODO: implement randomBytes()
}

extern "C" JNIEXPORT jbyteArray JNICALL
Java_com_example_rpo_1lr1_1test1_MainActivity_encrypt(JNIEnv *env, jclass, jbyteArray key, jbyteArray data)
{
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_com_example_rpo_1lr1_1test1_MainActivity_decrypt(JNIEnv *env, jclass, jbyteArray key, jbyteArray data)
{
    jsize ksz = env->GetArrayLength(key);
    jsize dsz = env->GetArrayLength(data);
...
Java_com_example_rpo_1lr1_1test1_MainActivity_CONSOS(JNIEnv *env, jobject thiz) {
    // TODO: implement CONSOS()
    LOG_INFO("New console inf: %d", 1007);
